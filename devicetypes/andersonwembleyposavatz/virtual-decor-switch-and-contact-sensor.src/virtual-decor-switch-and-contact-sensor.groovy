/**
 *  VIRTUAL DECOR Switch and Contact Sensor
 *
 ...
 */
metadata {
	definition (name: "VIRTUAL DECOR Switch and Contact Sensor", namespace: "andersonwembleyposavatz", author: "Anderson W Posavatz", vid: "generic-contact", runLocally: true , executeCommandsLocally: true , ocfDeviceType: "x.com.st.d.sensor.contact", 
 ) {
		capability "Actuator"
		capability "Switch"
		capability "Refresh"
	...
	tiles {
		...
		standardTile("open", "device.contact", inactiveLabel: false, decoration: "flat") {
			state "default", label:'${name}', icon:"st.contact.contact.open", backgroundColor:"#00A0DC"
		}
		standardTile("closed", "device.contact", inactiveLabel: false, decoration: "flat") {
			state "default", label:'${name}', icon:"st.contact.contact.closed", backgroundColor:"#cccccc"
			
        }
        
        standardTile("battery", "device.battery", inactiveLabel: false, decoration: "flat") {
             state "battery", label: "88", unit: "%"
       
       }

		main (["contact", "toggle"])
		details(["toggle", "contact", "on", "off", "open", "closed", "battery"])
	}
}
...
def initialize() {
	sendEvent(name: "switch", value: "on")
    sendEvent(name: "contact", value: "open")
    sendEvent(name: "battery", value: device.currentValue("battery") ?: 88)
